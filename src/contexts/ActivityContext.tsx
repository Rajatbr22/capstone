...
export const ActivityProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const getAllActivities = async () => {
      ...
      try {
        console.log('Fetching activities with URL:', `${API_URL}/activity/getActivities/${auth.user.id}`);
        console.log('Using token:', token ? `${token.substring(0, 5)}...` : 'No token');
        
        const response = await fetch(`${API_URL}/activity/getActivities/${auth.user.id}`, {
          method: 'GET',
          headers: {
        ...
        if (!result.success) {
          console.error('Failed to fetch activities:', result.message || 'Unknown error');
        ...
        if (Array.isArray(result.data)) {
          ...
          console.error('Expected an array of activities but received:', result.data);
    ...
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          console.log('Geolocation fetch successfully...', position.coords.latitude, position.coords.accuracy)
        },
        (err) => {
  ...
  const logActivity = async (
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/activity/updateActivities/${auth.user.id}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (!result.success) {
        console.error('Failed to log activity:', result.message || 'Unknown error');
      } else {
